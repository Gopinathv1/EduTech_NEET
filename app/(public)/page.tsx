...
export default function HomePage() {
  ...
  return (
    <>
      <section className="relative -mt-[73px] overflow-hidden bg-[#fff9f2] pt-[73px]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(15,157,154,0.18),transparent_28%),radial-gradient(circle_at_82%_20%,rgba(18,97,160,0.14),transparent_28%),linear-gradient(135deg,#fff9f2_0%,#ffffff_48%,#edf8f7_100%)]" />
        <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-[#0b1736]/8 to-transparent" />
        <div className="absolute -left-28 top-36 h-72 w-72 rounded-full bg-[#f6a623]/16 blur-3xl" />
        <div className="absolute -right-20 bottom-0 h-80 w-80 rounded-full bg-[#0f9d9a]/12 blur-3xl" />
        <PeacockQuillArtwork className="vv-peacock-rise right-[-280px] top-[20%] z-0 h-[420px] w-[610px] opacity-35 sm:right-[-190px] sm:h-[540px] sm:w-[780px] lg:right-[-50px] lg:top-[12%] lg:h-[650px] lg:w-[930px] lg:opacity-45" />
        ...
        <Container className="relative z-10 grid min-h-[calc(100vh-73px)] gap-12 py-20 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:py-28">
          <div className="vv-reveal max-w-4xl">
            <div className="inline-flex max-w-full items-center gap-3 rounded-full border border-[#eadcc7] bg-white/76 px-4 py-2 shadow-lg shadow-[#0b1736]/5">
              <span className="h-2 w-2 rounded-full bg-brand" />
              <p className="text-xs font-black uppercase tracking-[0.26em] text-[#14244a] sm:text-sm">Education • Preparation • Global Opportunity</p>
            </div>
            <h1 className="mt-7 max-w-5xl text-[clamp(3.6rem,8vw,8.8rem)] font-black uppercase leading-[0.88] text-[#0b1736]">
              {t('heroTitlePrefix')}
              <br />
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:max-w-3xl">
              {introItems.slice(0, 4).map((item) => (
                <div key={item} className="rounded-2xl border border-[#eadcc7] bg-white/78 p-4 shadow-lg shadow-[#0b1736]/5">
          ...
          <div className="relative min-h-[420px] lg:min-h-[620px]" aria-hidden="true">
            <div className="absolute bottom-8 left-0 right-0 mx-auto max-w-md rounded-[2rem] border border-[#eadcc7] bg-white/82 p-5 shadow-2xl shadow-[#0b1736]/12 backdrop-blur">
              <p className="text-xs font-black uppercase tracking-[0.22em] text-brand">Student Journey</p>
              <div className="mt-4 space-y-3">
                {JOURNEY.map((step, index) => (
                  <div key={step} className="flex items-center gap-3 rounded-2xl bg-[#fff9f2] p-3">
                    <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#0b1736] text-xs font-black text-white">
                      {index + 1}
                    </span>
                    <span className="font-black uppercase tracking-[0.08em] text-[#14244a]">{step}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="relative h-20 max-w-full overflow-hidden border-y border-[#eadcc7] bg-[#0b1736] [contain:layout_paint] sm:h-24">
      ...
      <Section id="what-we-do">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            ...
            <h2 className="mt-5 max-w-3xl text-[clamp(2.5rem,5vw,5.6rem)] font-black uppercase leading-[0.92] text-[#0b1736]">
              {t('quickActions.titleLine1')}
              <br />
              {t('quickActions.titleLine2')}
            </h2>
          </div>
          <p className="max-w-2xl text-base leading-8 text-slate-600">{t('quickActions.subtitle')}</p>
        </div>
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {quickActions.map((item, index) => (
            <div key={item.title} className="group rounded-[1.75rem] border border-[#eadcc7] bg-white p-6 shadow-xl shadow-[#0b1736]/5 transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#0b1736]/10 sm:p-7">
              ...
              <h3 className="mt-8 text-xl font-black uppercase leading-tight text-[#0b1736] xl:text-lg 2xl:text-xl">{item.title}</h3>
      ...
      <Section id="neet-preparation" tinted lazy>
        <div className="grid gap-10 lg:grid-cols-[0.86fr_1.14fr] lg:items-start">
          <div>
            ...
            <h2 className="mt-5 text-[clamp(3rem,6vw,6.8rem)] font-black uppercase leading-[0.88] text-[#0b1736]">
              NEET Preparation
            </h2>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-600">{t('neet.subtitle')}</p>
            <div className="mt-7 flex flex-wrap gap-2">
              <span className="rounded-full bg-[#0b1736] px-4 py-2 text-sm font-black uppercase tracking-wide text-white">NEET - Available Now</span>
              {FUTURE_EXAMS.map((exam) => (
                <span key={exam} className="rounded-full border border-[#eadcc7] bg-white px-4 py-2 text-sm font-semibold text-slate-600">
                  {exam} - Coming Soon
                </span>
              ))}
            </div>
          ...
          <div className="rounded-[2rem] border border-[#eadcc7] bg-white p-5 shadow-2xl shadow-[#0b1736]/8 sm:p-7">
            <div className="grid gap-3 sm:grid-cols-2">
              {PREP_FEATURES.map((feature) => {
                const Icon = feature.icon;
                return (
                  <div key={feature.title} className="flex items-center gap-3 rounded-2xl bg-[#fff9f2] p-4">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-soft text-brand">
                      <Icon className="h-5 w-5" />
                    </span>
                    <p className="text-sm font-black text-[#0b1736]">{feature.title}</p>
                  </div>
                );
              })}
            </div>
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              {subjects.map((subject) => (
                <Link key={subject.title} href={subject.href} className="rounded-2xl border border-[#eadcc7] bg-white p-5 transition hover:-translate-y-1 hover:border-brand/35 hover:shadow-lg">
                  <h3 className="text-xl font-black text-[#0b1736]">{subject.title}</h3>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {PREP_ACTIONS.map((action) => (
                <Link key={action.title} href={action.href} className="group rounded-2xl bg-[#0b1736] p-4 text-sm font-black uppercase tracking-[0.08em] text-white transition hover:bg-[#14244a]">
                  {action.title}
      ...
      <Section id="admissions" lazy>
        <div className="grid gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
          <div>
            ...
            <h2 className="mt-5 text-[clamp(2.5rem,5vw,5.8rem)] font-black uppercase leading-[0.92] text-[#0b1736]">
              {t('admissions.title')}
            </h2>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-600">{t('admissions.subtitle')}</p>
            <div className="mt-8">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {admissionCards.map((card, index) => (
              <Link key={card.title} href={card.href} className="rounded-[1.5rem] border border-[#eadcc7] bg-white p-6 shadow-xl shadow-[#0b1736]/5 transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#0b1736]/10">
                <p className="text-sm font-black text-brand">0{index + 1}</p>
                <h3 className="mt-5 text-2xl font-black uppercase leading-tight text-[#0b1736]">{card.title}</h3>
      ...
      <Section id="destinations" tinted lazy>
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            ...
            <h2 className="mt-5 text-[clamp(2.5rem,5vw,5.6rem)] font-black uppercase leading-[0.92] text-[#0b1736]">
              {t('destinations.title')}
            </h2>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-600">{t('destinations.subtitle')}</p>
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            {destinationCards.map((card) => {
              const meta = DESTINATION_META[card.title] ?? { label: 'GLOBAL', program: 'Education pathway' };
              return (
                <Link key={card.title} href={card.href} className="group rounded-[1.5rem] border border-[#eadcc7] bg-white p-6 shadow-xl shadow-[#0b1736]/5 transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#0b1736]/10">
                  <span className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-[#0b1736] text-sm font-black text-white" aria-hidden="true">{meta.label}</span>
                  <p className="mt-5 text-xs font-black uppercase tracking-[0.16em] text-brand">{meta.program}</p>
                  <h3 className="mt-2 text-xl font-black uppercase leading-tight text-[#0b1736]">{card.title}</h3>
      ...
      <Section id="student-journey" lazy>
        <div className="rounded-[2rem] bg-[#0b1736] p-6 text-white shadow-2xl shadow-[#0b1736]/16 sm:p-10 lg:p-12">
      ...
      <Section id="callback" tinted lazy>
        <div className="grid gap-10 rounded-[2rem] border border-[#eadcc7] bg-white p-6 shadow-2xl shadow-[#0b1736]/8 sm:p-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.32em] text-brand">{t('leadForm.eyebrow')}</p>
            <h2 className="mt-5 text-[clamp(2.5rem,5vw,5.6rem)] font-black uppercase leading-[0.92] text-[#0b1736]">
              {t('leadForm.title')}
            </h2>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-600">{t('leadForm.subtitle')}</p>
          </div>
          <HomeLeadForm />
        </div>
      </Section>
      ...
      <Section id="why-vv" lazy>
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {whyItems.map((item) => (
              <div key={item.title} className="rounded-[1.5rem] border border-[#eadcc7] bg-white p-6 shadow-xl shadow-[#0b1736]/5">
                <h3 className="text-lg font-black uppercase text-[#0b1736]">{item.title}</h3>
      ...
      <Section id="previous-year-papers" tinted lazy>
        <div className="grid gap-8 rounded-[2rem] border border-[#eadcc7] bg-white p-8 shadow-xl shadow-[#0b1736]/5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.28em] text-brand">NEET Previous Year Papers</p>
            <h2 className="mt-4 text-3xl font-black uppercase text-[#0b1736] sm:text-5xl">{t('pyq.title')}</h2>
            <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">{t('pyq.body')}</p>
          </div>
          <div>
      ...
      <Section lazy>
        <div className="rounded-[2rem] bg-gradient-to-br from-[#0b1736] via-[#14244a] to-[#0b1736] p-8 text-white shadow-2xl shadow-[#0b1736]/18 sm:p-12 lg:p-16">
