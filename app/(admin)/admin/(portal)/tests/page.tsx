              ...
              tests.map((t) => (
                <tr key={t.id} className="border-b border-slate-100 align-top">
                  <td className="px-4 py-3">
                    <p className="font-medium text-slate-900">{localizedName(t.title) || '(untitled)'}</p>
                    {t.availableLanguages.length > 1 ? (
                      <p className="text-xs text-slate-500">EN · TA</p>
                    ) : (
                      <p className="text-xs text-slate-500">EN</p>
                    )}
                  </td>
                  <td className="px-3 py-3 text-slate-600">{TYPE_LABEL[t.testType] ?? t.testType}</td>
                  <td className="px-3 py-3 text-slate-600">{t.isRandom ? 'Random' : 'Fixed'}</td>
                  <td className="px-3 py-3 text-slate-600">{t.totalQuestions}</td>
                  <td className="px-3 py-3 text-slate-600">₹{t.price}</td>
                  <td className="px-3 py-3">
                    {t.isPublished ? <Badge color="green">Published</Badge> : <Badge color="slate">Draft</Badge>}
                  </td>
                  <td className="px-3 py-3 text-slate-600">{t._count.attempts}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-start justify-end gap-3">
                      <Link
                        href={`/admin/tests/${t.id}`}
                        className="inline-flex items-center gap-1 pt-2 text-brand hover:text-brand-dark"
                      >
