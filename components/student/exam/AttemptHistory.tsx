...
export default async function AttemptHistory({ studentId, testId }: { studentId: string; testId: string }) {
  ...
  return <section className="mt-8 rounded-2xl border border-border p-5">
    ...
    <p className="mt-1 text-sm text-textSecondary">{t('used', { count: attempts.length, limit: FREE_ATTEMPT_LIMIT })}</p>
    ...
    {attempts.length < FREE_ATTEMPT_LIMIT && !attempts.some(a => a.status === 'IN_PROGRESS') ?
