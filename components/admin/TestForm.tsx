  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4">
          <p className="text-sm font-medium text-slate-700">Available languages</p>
          <div className="mt-1 flex gap-4 text-sm">
            <label className="flex items-center gap-2 text-slate-500">
              <input type="checkbox" checked disabled className="h-4 w-4 rounded border-slate-300" /> English
            </label>
            <label className="flex items-center gap-2 text-slate-700">
              <input type="checkbox" checked={f.ta} onChange={(e) => set('ta', e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-brand" /> Tamil
            </label>
          </div>
        </div>
      </section>
      ...
      {/* Difficulty mix */}
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        <h2 className="mb-1 text-base font-semibold text-slate-900">Difficulty mix</h2>
        <p className="mb-3 text-sm text-slate-600">Target percentage of each difficulty (must total 100%).</p>
      ...
      {/* Question selection */}
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        <h2 className="mb-1 text-base font-semibold text-slate-900">Questions</h2>
        {isFull ? (
          ...
          <div className="space-y-4">
            ...
            {!isFull && f.scope === 'SUBJECTS' ? (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {subjects.map((s) => (
                    <label key={s.id} className={`cursor-pointer rounded-lg border px-3 py-1.5 text-sm ${f.subjectIds.includes(s.id) ? 'border-brand bg-brand-soft text-brand' : 'border-slate-300 text-slate-700'}`}>
            ...
            {!isFull && f.scope === 'CHAPTERS' ? (
              <div className="space-y-3">
                {subjects.map((s) => (
                  <div key={s.id}>
                    <p className="mb-1 text-sm font-medium text-slate-700">{s.name}</p>
                    <div className="flex flex-wrap gap-2">
                      {s.chapters.map((c) => (
                        <label key={c.id} className={`cursor-pointer rounded-lg border px-2.5 py-1 text-xs ${f.chapterIds.includes(c.id) ? 'border-brand bg-brand-soft text-brand' : 'border-slate-300 text-slate-600'}`}>
      ...
      {/* Catalogue tags */}
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        <h2 className="mb-1 text-base font-semibold text-slate-900">Catalogue tags</h2>
        <p className="mb-3 text-sm text-slate-600">Optional — helps students filter this test by subject/chapter.</p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
