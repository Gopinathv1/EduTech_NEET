...
export default async function StudentDetailPage({ params }: Ctx) {
  ...
  return (
    <div>
      <Link href="/admin/students" className="text-sm font-medium text-brand hover:text-red-200">
        ← All students
      </Link>
      <div className="mt-2">
      ...
      {/* Profile */}
      <AdminCard>
        <h2 className="text-base font-semibold text-textPrimary">Profile</h2>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* Attempts + scores */}
        <AdminCard>
          <h2 className="text-base font-semibold text-textPrimary">Attempts &amp; scores</h2>
          {student.attempts.length === 0 ? (
            <p className="mt-2 text-sm text-textSecondary">No attempts yet.</p>
          ) : (
            <ul className="mt-3 divide-y divide-slate-100">
              {student.attempts.map((a) => (
                <li key={a.id} className="flex items-center justify-between gap-3 py-2.5">
                  <span className="min-w-0">
                    <span className="block truncate text-sm text-textPrimary">{localizedName(a.test.title, 'en')}</span>
                  ...
                  <span className="flex shrink-0 items-center gap-2">
                    ...
                    {a.result ? (
                      <span className="text-sm font-bold text-textPrimary" title={`out of ${a.result.totalQuestions * 4}`}>
                        {a.result.score}
                      </span>
                    ) : (
        ...
        {/* Payments */}
        <AdminCard>
          <h2 className="text-base font-semibold text-textPrimary">Payments</h2>
          <p className="mt-0.5 text-xs text-textSecondary">₹{paid} paid across {student.payments.filter((p) => p.status === 'SUCCESS').length} test(s).</p>
          {student.payments.length === 0 ? (
            <p className="mt-2 text-sm text-textSecondary">No payments yet.</p>
          ) : (
            <ul className="mt-3 divide-y divide-slate-100">
              {student.payments.map((p) => (
                <li key={p.id} className="flex items-center justify-between gap-3 py-2.5">
                  <span className="min-w-0">
                    <span className="block truncate text-sm text-textPrimary">{localizedName(p.test.title, 'en')}</span>
                    <span className="text-xs text-slate-400">
                  ...
                  <span className="flex shrink-0 items-center gap-2">
                    <span className="text-sm font-medium text-textSecondary">₹{p.amount}</span>
      ...
      <AdminCard className="mt-6">
        <h2 className="text-base font-semibold text-textPrimary">Consultancy leads</h2>
        {student.leads.length === 0 ? (
          <p className="mt-2 text-sm text-textSecondary">No leads.</p>
        ) : (
            ...
            {student.leads.map((l) => (
              <li key={l.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border px-3 py-2 text-sm">
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-textSecondary">{l.interestedCountry ? localizedName(l.interestedCountry.name, 'en') : '—'}</span>
                </span>
